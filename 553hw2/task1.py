...
if case_number == 1:
    user_business_rdd = user_business_rdd.map(lambda u_b: (u_b[0], u_b[1])).distinct()
else:
    user_business_rdd = user_business_rdd.map(lambda u_b: (u_b[1], u_b[0])).distinct()
user_business_basket = user_business_rdd.groupByKey().map(lambda x: (x[0], list(x[1])))
total_baskets = user_business_basket.count()
def find_candidates(iterator):
    ...
    for singleton, count in freq_singles.items():
        if count >= math.ceil(support * len(baskest_list) / total_baskets):
            singletons.append(str(singleton))
    i = 1
    candidates = dict()
    while True:
        candidates[i] = []
                    ...
                    for pretup in pre_tuples:
                        for item in sorted_item:
                            if pretup < item:
                                ...
                                possib_tuple2.append(appendpretup)
            else:
                for pre_tuple in pre_freq:
                    ...
                    for pretup in pre_tuples:
                        for item in sorted_item:
                            if pretup[-1] < item:
                                ...
                                possib_tuple2.append(appendpretup)
        for tup in possib_tuple2:
            tuples_count[tup] = 0
            for basket in baskest_list:
                if set(tup).issubset(basket):
                    tuples_count[tup] += 1
        # check if tuples whose subtuple all frequent are frequent themselves by their appearance times in all baskets
        for tup, count in tuples_count.items():
            if count >= math.ceil(support * len(baskest_list) / total_baskets):
                candidates[i].append(tuple(sorted(tup)))
...
candidate_output = user_business_basket.map(lambda x: x[1]) \
    ...
    for basket in iterator:
        for candidate in list(chain.from_iterable(candidates_result)):
            if set(candidate).issubset(basket):
                yield candidate
freq_output = user_business_basket.map(lambda x: x[1])\
